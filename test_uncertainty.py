...
class ECELoss(nn.Module):
    ...
    def forward(self, logits, labels, temperature=1.0, onevsrest=False):
        ...
        if onevsrest:
            softmaxes = torch.sigmoid(logits_scaled) / torch.sum(
                torch.sigmoid(logits_scaled), dim=1, keepdim=True
            )
        else:
    ...
    if params.method in [
        ...
        if params.adaptation:
            model.task_update_num = (
                100  # We perform adaptation on MAML simply by updating more times.
            )
        model.eval()
        ...
        for i, (x, _) in enumerate(novel_loader):
            ...
            targets = torch.tensor(np.repeat(range(params.test_n_way), model.n_query))
        ...
        for i in range(iter_num):
            ...
            targets = torch.tensor(np.repeat(range(n_way), n_query))
...
def main():
    ...
    for _ in range(repeat):  # repeat):
        ...
        temperature = ece_module.calibrate(
            logits, targets, iterations=300, lr=0.01
        ).item()
        if temperature > 0:
