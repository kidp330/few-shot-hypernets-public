...
class ECELoss(nn.Module):
    ...
    def calibrate(self, logits, labels, iterations=50, lr=0.01):
        temperature_raw = torch.ones(1, requires_grad=True)
        nll_criterion = nn.CrossEntropyLoss()
        optimizer = torch.optim.LBFGS(
            [temperature_raw], lr=lr, max_iter=iterations)
        ...
        def closure():
            ...
            loss = nll_criterion(
                logits / temperature_raw.expand_as(logits), labels)
    ...
    def forward(self, logits, labels, temperature=1.0, onevsrest=False):
        ...
        if (onevsrest):
            softmaxes = torch.sigmoid(
                logits_scaled) / torch.sum(torch.sigmoid(logits_scaled), dim=1, keepdim=True)
        else:
...
def get_logits_targets(params):
    ...
    if params.method in ['maml', 'maml_approx', 'DKT']:
        ...
        for i, (x, _) in enumerate(novel_loader):
            ...
            targets = torch.tensor(
                np.repeat(range(params.test_n_way), model.n_query))
...
def main():
    ...
    for _ in range(repeat):  # repeat):
        ...
        temperature = ece_module.calibrate(
            logits, targets, iterations=300, lr=0.01).item()
        if (temperature > 0):
            temperature_list.append(temperature)
        print("Calibration: temperature", temperature,
              "; mean temperature", np.mean(temperature_list))
    ...
    for i in range(seed, seed+repeat):
        ...
        ece = ece_module.forward(
            logits, targets, temperature, onevsrest=False).item()
        ece_list.append(ece)
        print("ECE:", np.mean(ece_list), "+-", np.std(ece_list))

    # 3. Print the final ECE (averaged over all seeds)
    print("-----------------------------")
    print('Seeds = %d | Overall ECE = %4.4f +- %4.4f' %
          (repeat, np.mean(ece_list), np.std(ece_list)))
    print("-----------------------------")
