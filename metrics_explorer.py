...
root = Path(
    "/home/mprzewie/coding/gmum_mnt/uj/few-shot-hypernets/save/checkpoints/cross_char/"
)
...
def load_experiment(experiment_path: Path) -> Tuple[Dict, pd.DataFrame]:
    ...
    with (experiment_path / ARGS_FILE).open("r") as f:
        args = json.load(f)
    return args, metrics_dict_to_df(
        experiment_name=experiment_path.name, metrics_dict=metrics, args_dict=args
    )


loggable_experiments = {
    p.parent.name: p
    for p in root.glob(f"*/{METRICS_FILE}")
    if (p.parent / ARGS_FILE).exists()
}
...
selected_metric = st.selectbox(
    "Select metric",
    available_metrics,
    index=available_metrics.index("accuracy_val_max"),
)
...
st.altair_chart(
    ...
    .encode(
        ...
        color="exp_name",
        tooltip=["exp_name", "value", "epoch"]
        + [a for a in all_args if a.startswith(HN_PREFIX)],
    )
        ...
        with st.expander(f"{a} := {unique_as}", expanded=(len(unique_as) > 1)):
            st.altair_chart(
                ...
                .encode(
                    x="epoch",
                    y=alt.Y(
                        "value", aggregate=("mean" if aggregate_y else alt.Undefined)
                    ),
                    color="exp_name",
                    tooltip=["exp_name", "value", "epoch"]
                    + [a for a in all_args if a.startswith(HN_PREFIX)],
