...
from typing import Any, Literal, Sized, Type, List, Union, Dict, Optional
...
from io_utils import model_dict, get_resume_file, setup_neptune
...
def train(base_loader, val_loader, model, optimization, start_epoch, stop_epoch, params: TrainParams, *,
    ...
    if optimization == 'adam':
        ...
        raise ValueError(f'Unknown optimization {optimization}, please define by yourself')  # nopep8
    ...
    if (params.checkpoint_dir / "metrics.json").exists() and params.resume:
        with (params.checkpoint_dir / "metrics.json").open("r") as f:
            try:
                        ...
                        for i in range(params.hn_val_epochs + 1):
                            if i != 0:
                                max_acc_adaptation_dict[f"accuracy/val_support_max@-{i}"] = \
                                    metrics_per_epoch[f"accuracy/val_support_max@-{
                                        i}"][-1]
    ...
    for epoch in range(start_epoch, stop_epoch):
        if epoch >= params.es_epoch:
            if max_acc < params.es_threshold:
                print("Breaking training at epoch", epoch, "because max accuracy", max_acc, "is lower than threshold",
                      params.es_threshold)
                break
        # __jm__ commenting out for hyperMAML, might be required by other models
        # model.epoch = epoch
        # model.start_epoch = start_epoch
        # model.stop_epoch = stop_epoch

        model.train()
        if isinstance(model, (HyperMAML, BayesHMAML)):
            metrics = model.train_loop(base_loader, optimizer)
        else:
        ...
        if (epoch % params.eval_freq == 0) or epoch in [
            ...
            acc = 100 / n_way + epoch * 0.001 + random.normalvariate()
            test_loop_metrics = dict()
            ...
            if params.test_set_forward_with_adaptation:
                for i in range(params.val_epochs + 1):
                    if i != 0:
                        metrics[f"accuracy/val_support_max@-{i}"] = max_acc_adaptation_dict[
                            f"accuracy/val_support_max@-{i}"]
                    metrics[f"accuracy/val_max@-{i}"] = max_acc_adaptation_dict[f"accuracy/val_max@-{i}"]  # nopep8
            ...
            if params.test_set_forward_with_adaptation:
                for i in range(params.val_epochs + 1):
                    ...
                    if metrics[f"accuracy/val@-{i}"] > max_acc_adaptation_dict[f"accuracy/val_max@-{i}"]:
                        max_acc_adaptation_dict[f"accuracy/val_max@-{i}"] = metrics[f"accuracy/val@-{i}"]  # nopep8
            ...
            if params.maml_save_feature_network and isinstance(model, (MAML, HyperMAML, BayesHMAML)):
                outfile = os.path.join(
                    params.checkpoint_dir, 'last_feature_net.tar')
                torch.save(
                    {'epoch': epoch, 'state': model.feature.state_dict()}, outfile)
...
def get_scheduler(params, optimizer, stop_epoch=None) -> LRScheduler:
    if params.lr_scheduler == "multisteplr":
        if params.milestones is not None:
            milestones = params.milestones
        else:
            milestones = list(range(0, params.stop_epoch,
        ...
        return lr_scheduler.MultiStepLR(optimizer,
                                        milestones=list(
                                            range(0, params.stop_epoch, params.stop_epoch // 4))[1:],
                                        gamma=1)
...
if __name__ == '__main__':
    ...
    def stop_epoch_default():
        if params.method in ['baseline', 'baseline++']:
            ...
            if params.dataset in ['CUB']:
                ...
                return 200
            if params.dataset in ['miniImagenet', 'cross']:
                return 400
            return 400  # default
        if params.n_shot == 1:
            return 600
    ...
    if params.method in ['baseline', 'baseline++']:
        ...
        if params.method == 'baseline':
            ...
            model = BaselineTrain(
                model_dict[params.model], params.num_classes, loss_type='dist')
                           ...
                           'maml_approx', 'hyper_maml', 'bayes_hmaml']:
        BayesHMAML = HyperMAML
        ...
        if (params.method == 'DKT'):
            ...
            model = MAML(model_dict[params.model], n_way, n_support, n_query, params, approx=(
        ...
        elif params.method in ["hyper_maml", "bayes_hmaml"]:
