...
def train(base_loader, val_loader, model, optimization, start_epoch, stop_epoch, params, *,
    ...
    if optimization == 'adam':
        ...
        raise ValueError(f'Unknown optimization {
                         optimization}, please define by yourself')
        ...
        if (epoch % params.eval_freq == 0) or epoch in [
            ...
            if params.hm_set_forward_with_adaptation:
                for i in range(params.hn_val_epochs + 1):
                    ...
                    metrics[f"accuracy/val_max@-{
                        i}"] = max_acc_adaptation_dict[f"accuracy/val_max@-{i}"]
            ...
            if params.hm_set_forward_with_adaptation:
                for i in range(params.hn_val_epochs + 1):
                    ...
                    if metrics[f"accuracy/val@-{i}"] > max_acc_adaptation_dict[f"accuracy/val_max@-{i}"]:
                        max_acc_adaptation_dict[f"accuracy/val_max@-{
                            i}"] = metrics[f"accuracy/val@-{i}"]
...
if __name__ == '__main__':
    ...
    if params.method in ['baseline', 'baseline++']:
        ...
        train_few_shot_params = dict(
            n_way=params.train_n_way, n_support=params.n_shot, n_query=n_query)
