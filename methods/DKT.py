...
class DKT(MetaTemplate):
    ...
    def train_loop(self, epoch, train_loader, optimizer, print_freq=10):
        ...
        for i, (x, _) in enumerate(train_loader):
            ...
            y_all = torch.repeat_interleave(
                range(self.n_way), self.n_query + self.n_support
            )
            x_support = (
                x[:, : self.n_support, :, :, :]
                .contiguous()
            ...
            _train_list = [z_train] * self.n_way
            lenghtscale = 0.0
            noise = 0.0
            outputscale = 0.0
            for idx, single_model in enumerate(self.model.models):
    ...
    def correct(self, x, N=0, laplace=False):
        ...
        if laplace:
            ...
            _accuracy = (np.sum(y_pred == y_query) / float(len(y_query))) * 100.0
    ...
    def test_loop(self, test_loader, _record=None, return_std=False):
        _print_freq = 10
        _correct = 0
        _count = 0
        acc_all = []
