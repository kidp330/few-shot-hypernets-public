...
class DKT(MetaTemplate):
    ...
    def get_model_likelihood_mll(self, train_x_list=None, train_y_list=None):
        if(train_x_list is None): train_x_list=[torch.ones(100, 64).to(device())]*self.n_way
        if(train_y_list is None): train_y_list=[torch.ones(100).to(device())]*self.n_way
        model_list = list()
        ...
        self.model = gpytorch.models.IndependentModelList(*model_list).to(device())
        self.likelihood = gpytorch.likelihoods.LikelihoodList(*likelihood_list).to(device())
        self.mll = gpytorch.mlls.SumMarginalLogLikelihood(self.likelihood, self.model).to(device())
    ...
    def train_loop(self, epoch, train_loader, optimizer, print_freq=10):
        ...
        for i, (x,_) in enumerate(train_loader):
            ...
            x_all = x.contiguous().view(self.n_way * (self.n_support + self.n_query), *x.size()[2:]).to(device())
            y_all = Variable(torch.from_numpy(np.repeat(range(self.n_way), self.n_query+self.n_support)).to(device()))
            x_support = x[:,:self.n_support,:,:,:].contiguous().view(self.n_way * (self.n_support), *x.size()[2:]).to(device())
            y_support = np.repeat(range(self.n_way), self.n_support)
            x_query = x[:,self.n_support:,:,:,:].contiguous().view(self.n_way * (self.n_query), *x.size()[2:]).to(device())
    ...
    def correct(self, x, N=0, laplace=False):
        ...
        x_support = x[:,:self.n_support,:,:,:].contiguous().view(self.n_way * (self.n_support), *x.size()[2:]).to(device())
        y_support = torch.from_numpy(np.repeat(range(self.n_way), self.n_support)).to(device())
        x_query = x[:,self.n_support:,:,:,:].contiguous().view(self.n_way * (self.n_query), *x.size()[2:]).to(device())
        y_query = np.repeat(range(self.n_way), self.n_query)
