...
class DKT(MetaTemplate):
    def __init__(self, model_func, n_way, n_support):
        ...
        if (kernel_type == "cossim"):
            ...
            self.feature_extractor.trunk.add_module(
                "bn_out", nn.BatchNorm1d(latent_size))
        else:
    ...
    def get_model_likelihood_mll(self, train_x_list=None, train_y_list=None):
        if (train_x_list is None):
            train_x_list = [torch.ones(100, 64)]*self.n_way
        if (train_y_list is None):
            train_y_list = [torch.ones(100)]*self.n_way
        model_list = list()
    ...
    def _reset_variational(self):
        ...
        for idx, param in enumerate(self.gp_layer.variational_parameters()):
            if (idx == 0):
                ...
                raise ValueError(
                    '[ERROR] DKT the variational_parameters at index>1 should not exist!')
        ...
        if (self.leghtscale_list is None):
            ...
            for idx, single_model in enumerate(self.model.models):
                ...
                self.noise_list.append(
                    single_model.likelihood.noise.clone().detach())
                self.outputscale_list.append(
                    single_model.covar_module.outputscale.clone().detach())
        else:
            for idx, single_model in enumerate(self.model.models):
                single_model.covar_module.base_kernel.lengthscale = self.leghtscale_list[idx].clone(
                ).detach()  # .requires_grad_(True)
                single_model.likelihood.noise = self.noise_list[idx].clone(
                ).detach()
                single_model.covar_module.outputscale = self.outputscale_list[idx].clone(
                ).detach()
    ...
    def train_loop(self, epoch, train_loader, optimizer, print_freq=10):
        ...
        for i, (x, _) in enumerate(train_loader):
            ...
            with torch.no_grad():
                ...
                predictions = self.likelihood(*self.model(*z_support_list))
                predictions_list = list()
                for gaussian in predictions:
                    predictions_list.append(torch.sigmoid(
                        gaussian.mean).cpu().detach().numpy())
                y_pred = np.vstack(predictions_list).argmax(
                    axis=0)  # [model, classes]
                accuracy_support = (
                    np.sum(y_pred == y_support) / float(len(y_support))) * 100.0
                if (self.writer is not None):
                ...
                # return 20 MultiGaussian Distributions
                predictions = self.likelihood(*self.model(*z_query_list))
                predictions_list = list()
                for gaussian in predictions:
                    predictions_list.append(torch.sigmoid(
                        gaussian.mean).cpu().detach().numpy())
                y_pred = np.vstack(predictions_list).argmax(
                    axis=0)  # [model, classes]
                accuracy_query = (np.sum(y_pred == y_query) /
                                  float(len(y_query))) * 100.0
                if (self.writer is not None):
                    self.writer.add_scalar(
            ...
            if i % print_freq == 0:
                if (self.writer is not None):
                    self.writer.add_histogram(
                        'z_support', z_support, self.iteration)
                print('Epoch [{:d}] [{:d}/{:d}] | Outscale {:f} | Lenghtscale {:f} | Noise {:f} | Loss {:f} | Supp. {:f} | Query {:f}'.format(
                    epoch, i, len(train_loader), outputscale, lenghtscale, noise, loss.item(), accuracy_support, accuracy_query))
    ...
    def correct(self, x, N=0, laplace=False):
        ...
        if (laplace):
            ...
            if (self.normalize):
                z_support = F.normalize(z_support, p=2, dim=1)
            gp.fit(z_support.cpu().detach().numpy(),
                   y_support.cpu().detach().numpy())
            z_query = self.feature_extractor.forward(x_query).detach()
            if (self.normalize):
                z_query = F.normalize(z_query, p=2, dim=1)
            y_pred = gp.predict(z_query.cpu().detach().numpy())
            accuracy = (np.sum(y_pred == y_query) /
                        float(len(y_query))) * 100.0
    ...
    def test_loop(self, test_loader, record=None, return_std=False):
        ...
        for i, (x, _) in enumerate(test_loader):
            ...
            if (i % 100 == 0):
                ...
                print('Test | Batch {:d}/{:d} | Loss {:f} | Acc {:f}'.format(
    ...
    def get_logits(self, x):
        ...
        with torch.no_grad(), gpytorch.settings.num_likelihood_samples(32):
            ...
            for gaussian in predictions:
                # .cpu().detach().numpy())
                predictions_list.append(gaussian.mean)
...
class ExactGPLayer(gpytorch.models.ExactGP):
    ...
    def __init__(self, train_x, train_y, likelihood, kernel='linear'):
        ...
        # Polynomial (p=2)
            ...
            raise ValueError("[ERROR] the kernel '" +
                             str(kernel) + "' is not supported!")
